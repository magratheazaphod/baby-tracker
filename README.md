...
| `DEMO_MODE` | unset | `1` makes the instance a read-only public demo: login and reads work, every write, upload, push and export is refused, and an empty database is filled with synthetic data at boot (needs `BIRTH_DATE`) |
| `DEMO_RESEED` | unset | `1` regenerates the demo data on every start; only honoured with `DEMO_MODE` |
